  ...
  function draw() {
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 120) {
          ctx.beginPath();
          ctx.strokeStyle = `rgba(1,101,177,${0.06 * (1 - dist / 120)})`;
          ctx.lineWidth = 0.5;
    ...
    particles.forEach(p => {
      ...
      ctx.fillStyle = `rgba(1,101,177,${p.alpha})`;
      ctx.fill();
      p.x += p.vx;
      p.y += p.vy;
...
function animatePrices() {
  ...
  priceEls.forEach(el => {
    ...
    setTimeout(() => { el.style.color = '#0165b1'; }, 600);
  });
}
...
(function initTestimonials() {
  ...
  track.innerHTML = cards + cards + cards; // Triple for safe coverage

  // Note: 3D Tilt interaction removed as requested. Hover effects are handled by CSS.
})();
